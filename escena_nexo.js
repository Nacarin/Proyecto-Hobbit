...
export class EscenaNexo extends Escena {
  ...
  decision () {
    /**
         ...
         */
    const escenaElegida = super.hacerPregunta('Deseas tener un sueño sobre gundam (1), un sueño en un isekai (2) o despertar (3)')
    switch (escenaElegida) {
      case '1':{
        Flags.activarFlagGundam()
        const aventuraGundam = new AventuraGundam()
      ...
      case '2':{
        ...
        const aventuraIsekai = new AventuraIsekai()
        aventuraIsekai.iniciarSuenho(this.aventura)
        break
      }
      case '3':{
        if (Flags.flagGundam && Flags.flagIsekai) {
          const escena9 = new Escena9(this.aventura)
